...
## 12. End-to-End Fanpage Autopilot (1-Prompt System)

Workflow này gộp toàn bộ các kỹ năng (Brand Voice, Copywriting, Asset Sourcing) thành một luồng chạy TỰ ĐỘNG khi nhận lệnh cơ bản từ người dùng.

### 12.1 Kích hoạt Workflow (Master Prompt)

Khi User nhập lệnh cơ bản như: *"Giúp tôi xây kênh bán cái điện thoại iPhone 16 này"* hoặc *"Tạo page cho dịch vụ thiết kế Automation"*, Agent sẽ kích hoạt luồng **5-Step End-To-End**:

1. **Step 1: Onboarding & Profiling** (Xác định ngách, thông tin nền).
2. **Step 2: Dynamic Tone Classifier** (Chọn Framework viết từ `comarai-copywriting`).
3. **Step 3: Content Pillar Matrix** (Lên chiến lược chuyển đổi).
4. **Step 4: Asset Generation** (Phất cờ yêu cầu Meme hoặc Ảnh thật).
5. **Step 5: Automation Export** (Đóng gói JSON để schedule).

### 12.2 Logic Dynamic Tone Classifier (Khớp Văn Phong)

Agent sẽ tự động **định tuyến** bài viết dựa trên phân tích sản phẩm ở Step 1, không cần hỏi lại user:

| Phân Loại Sản Phẩm | Framework Copywriting Ưu Tiên | Giải Thích |
|-------------------|-------------------------------|------------|
| Sản phẩm bán lẻ (Gadget, Đồ gia dụng) | **PAS** (Pain-Agitate-Solve) / **Feature-Benefit** | Nhấn mạnh sự tiện lợi, nỗi đau nhỏ hằng ngày, và action chốt đơn nhanh. |
| Dịch vụ / Khoá học (High-Ticket) | **PASTOR** / **BAB** (Before-After-Bridge) | Kể chuyện, xây dựng niềm tin dài hạn, nhắm vào sự thay đổi trước-sau. |
| B2B / SaaS Automation | **Case Study** / **AIDA** | Dựa trên ROI, data thực tế, tối ưu hoá thời gian. |
| Giải trí / Fanpage Cộng Đồng | **Meme-focused** / **Controversy Hook** | Bài cực ngắn, hình ảnh là chủ đạo, text mang tính khơi mào tranh luận. |

### 12.3 Logic Tự Động Chọn Hình Ảnh (Asset Routing)

Với mỗi post được sinh ra, Agent PHẢI đính kèm 1 block `[ASSET_REQUEST]` cho hệ thống (n8n/Make) biết phải call API nào:

```yaml
# Ví dụ: Bài viết về nỗi đau khi phải nhập data bằng tay (Dịch vụ Automation)
[ASSET_REQUEST]
- TYPE: "Meme"
- SOURCE: "Imgflip API"
- TEMPLATE: "Drake Hotline Bling"
- TEXT_TOP: "Thuê thêm nhân viên nhập data"
- TEXT_BOTTOM: "Dùng comarai-content-engine chạy tự động"

# Ví dụ: Post khoe thiết kế đẹp của điện thoại iPhone 16
[ASSET_REQUEST]
- TYPE: "Stock Photo"
- SOURCE: "Unsplash / Pexels API"
- KEYWORDS: "Person holding smartphone in cafe, aesthetic, morning light"

# Ví dụ: Khoe thành quả Trade / Lên Camp
[ASSET_REQUEST]
- TYPE: "AI Generation"
- SOURCE: "Midjourney/Flux"
- PROMPT: "Messy desk, glowing secondary monitor showing chart lines, half empty coffee, late night, candid smartphone shot, slight noise, anti slop --ar 16:9"
```

### 12.4 Đóng Gói Automation (JSON cho n8n/Buffer)

Kết quả cuối cùng của Agent (sau khi xây chiến lược và viết xong các bài) không chỉ là text đọc cho vui, mà phải format gọn gàng chuẩn Webhook để đẩy vào hệ thống Automation (lên lịch đăng):

```json
{
  "project": "Fanpage Bán iPhone 16 Lướt",
  "schedule_type": "Daily",
  "posts": [
    {
      "day": "1",
      "platform": "Facebook",
      "pillar_type": "Awareness",
      "caption": "Bạn luôn phải vừa sạc vừa dùng vì pin điện thoại cũ đã chai nát? [PAS Framework body]...",
      "asset": {
         "type": "meme",
         "imgflip_template_id": "87743020",
         "text_0": "Thay pin máy cũ",
         "text_1": "Lên hẳn iPhone 16 lướt tiết kiệm sạc 1 lần dùng cả ngày"
      }
    },
    {
      "day": "2",
      "platform": "Facebook",
      "pillar_type": "Consideration",
      "caption": "Mở hộp iPhone 16 bản Titan rẻ hơn máy mới 3 triệu thì trông nhan sắc thế nào?...",
      "asset": {
         "type": "photo_search",
         "keywords": "iphone holding naturally cafe, modern"
      }
    }
  ]
}
```
*Note cho User: Khi nhận block JSON này, bạn có thể truyền thẳng qua webhook của n8n hoặc Make.com. API của n8n sẽ đọc object `asset`, tự đi call Imgflip lấy meme, hoặc call Unsplash lấy ảnh, rải lịch vào Buffer.*
